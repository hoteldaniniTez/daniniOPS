...
import {
    Search, CalendarDays, Edit3, LinkIcon, XCircle, Sparkles, CreditCard, Wallet,
    X, AlertTriangle, Trash2, Calculator, Landmark, Heart, Gift, Info, ArrowUpRight, User,
    MinusCircle,
    Clock
} from 'lucide-react';
import { MetodoPago, TerminalTipo, TipoTarjeta } from '@/lib/generated/prisma/enums';
import { useUIStore, usePagoStore } from '@/store'; // 🟢 Importamos el pagoStore
import { AreaGroup, KPICard, MiniCard } from '../reception/movement-ui';
import { MovimientoCompleto, METODOS_PAGO_OPCIONES, TERMINALES, MESEROS, AreasEvento, TipoEvento, METODOS_CON_FACTURA, AREAT } from '@/interfaces'; // 🟢 Importamos tus interfaces
import { getDailyCorteMovements, updateMovement, deleteMovement, searchMovements } from "@/actions";
...
export const DailyCorteView = ({ initialMovements }: { initialMovements: any[] }) => {
    ...
    const [fechaInicio, setFechaInicio] = useState(() => new Date().toLocaleDateString("en-CA", { timeZone: "America/Mexico_City" }));
    const [fechaFin, setFechaFin] = useState(() => new Date().toLocaleDateString("en-CA", { timeZone: "America/Mexico_City" }));

    // 🟢 CONFIGURACIÓN REACT-HOOK-FORM (Misma que en CorteClientView)
    ...
    useEffect(() => {
        if (montoNetoEditado !== undefined) setTotalADebuer(Number(montoNetoEditado));
    ...
    useEffect(() => {
        if (selectedMovement) setTotalADebuer(Number(selectedMovement.montoNeto));
    }, [selectedMovement, setTotalADebuer]);

    // Efecto para cargar datos al abrir modo edición
    useEffect(() => {
        if (selectedMovement && isEditing) {
            reset({
                ...
                pagos: selectedMovement.pagos.map((p: any) => ({
                    id: p.id, // <-- CRÍTICO: Pasamos el ID real de la BD
                    metodo: p.metodo,
    ...
    const handleDateSearch = async () => {
        // 1. Validamos que el usuario no haga cosas extrañas
        if (!fechaInicio || !fechaFin) {
            toast.error("Selecciona un rango de fechas para consultar");
            return;
        }
        ...
        // 2. Pasamos las dos fechas al Action
        const res = await getDailyCorteMovements(fechaInicio, fechaFin);

        if (res.ok) {
            if (res.movements) setMovements(res.movements as any);

            // UX: Le avisamos al usuario si ese día/rango estuvo vacío
            if (res.movements?.length === 0) {
                toast.info("No hay movimientos registrados en este periodo");
            } else {
                // Limpiamos el buscador si es que había texto escrito
                setSearchTerm("");
            }
        } else {
            toast.error(res.message || "Error al obtener los datos");
        }
    ...
    const onUpdateSubmit = async (data: any) => {
        ...
        if (res.ok) {
            ...
            handleDateSearch(); // Refrescamos el día
            setIsEditing(false);
            closeDetail();
        } else {
            toast.error(res.message || "Error al generar el reporte", { id: loadingToast });
        }
    };
    ...
    const handleGlobalSearch = async (term: string) => {
        ...
        // Validaciones de UX/Negocio
        if (!fechaInicio || !fechaFin) {
            toast.error("Selecciona un rango de fechas para buscar");
            return;
        }
        ...
        if (cleanTerm.length === 0) {
            toast.info("Ingresa un folio o nombre para buscar en este periodo");
            return;
        }
        ...
        setLoading(true);
        // Pasamos las fechas al action
        const res = await searchMovements(cleanTerm, fechaInicio, fechaFin);

        if (res.ok) {
            setMovements(res.movements as any);
            if (res.movements.length === 0) {
                toast.info("No se encontraron coincidencias en este periodo de tiempo");
            }
        } else {
    ...
    return (
        <div className={`space-y-6 max-w-400 mx-auto p-4 transition-all ${loading ? 'opacity-50 blur-sm' : ''}`}>
            ...
            <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 mb-8">
                ...
                {/* TÍTULO: Siempre visible y bien espaciado */}
                <div className="space-y-1">
                    <h1 className="text-2xl md:text-3xl font-black italic uppercase text-zinc-900 flex items-center gap-3 tracking-tighter">
                        <CalendarDays className="text-emerald-600 shrink-0" size={28} />
                        <span>Corte General de Caja</span>
                    </h1>
                    <p className="text-[10px] md:text-xs font-bold text-zinc-400 uppercase tracking-[0.2em] ml-1">
                        Auditoría Operativa
                    </p>
                </div>

                {/* PANEL DE CONTROL UNIFICADO: Fechas y Buscador */}
                <div className="w-full lg:w-auto bg-white p-3 md:p-4 rounded-4xl border border-zinc-200 shadow-sm">
                    <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center">

                        {/* 1. SECCIÓN DE FECHAS */}
                        <div className="flex flex-col sm:flex-row items-end gap-3 w-full lg:w-auto">
                            {/* Contenedor de Inputs de Fecha (Grid en móvil, Flex en desktop) */}
                            <div className="grid grid-cols-2 sm:flex gap-3 w-full sm:w-auto">
                                <div className="flex flex-col w-full sm:w-36">
                                    ...
                                    <input
                                        ...
                                        className="w-full px-3 py-2.5 bg-zinc-50 rounded-2xl text-xs font-bold text-zinc-700 outline-none focus:ring-2 focus:ring-zinc-900/10 border border-transparent transition-all cursor-pointer"
                                    />
                                </div>
                                <div className="flex flex-col w-full sm:w-36">
                            ...
                            <button
                                ...
                                className="w-full sm:w-auto flex items-center justify-center gap-1.5 px-5 py-2.5 bg-zinc-900 hover:bg-black text-white rounded-2xl transition-all shadow-md active:scale-95 text-[10px] font-black uppercase tracking-wider whitespace-nowrap cursor-pointer h-9.5"
                                title="Ver corte completo del periodo"
                            >
                        ...
                        {/* SEPARADOR VISUAL (Vertical en PC, Horizontal en Móvil/Tablet) */}
                        <div className="hidden lg:block w-px h-10 bg-zinc-200" />
                        <div className="block lg:hidden w-full h-px bg-zinc-100 my-1" />

                        {/* 2. SECCIÓN DE BÚSQUEDA ESPECÍFICA */}
                        <div className="flex flex-col sm:flex-row items-end gap-3 w-full lg:w-auto">
                            <div className="relative w-full sm:flex-1 lg:w-60">
                                ...
                                <input
                                    ...
                                    onKeyDown={(e) => e.key === 'Enter' && handleGlobalSearch(searchTerm)}
                                    className="w-full pl-9 pr-4 py-2.5 bg-zinc-50 rounded-2xl text-xs font-bold text-zinc-700 outline-none focus:ring-2 focus:ring-emerald-500/20 border border-transparent transition-all h-9.5"
                                />
                            </div>

                            <button
                                onClick={() => handleGlobalSearch(searchTerm)}
                                className="w-full sm:w-auto flex items-center justify-center gap-1.5 px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl transition-all shadow-md active:scale-95 text-[10px] font-black uppercase tracking-wider whitespace-nowrap cursor-pointer h-9.5"
                            >
                                Buscar
                            </button>
                        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <KPICard title="Ventas Netas" amount={totals.ingresoNeto} icon={<ArrowUpRight />} color="bg-zinc-900" subtitle="Ingreso Real del Día" />
                <KPICard title="Bolsa de Propinas" amount={totals.propinas} icon={<Sparkles />} color="bg-amber-500" subtitle="Total Personal" />
                <KPICard title="Gran Total en Caja" amount={totals.granTotalCaja} icon={<Calculator />} color="bg-emerald-600" isMain subtitle="Ingreso Físico del Día" />
            </div>
            ...
            <div className="bg-white rounded-[2.5rem] border border-zinc-200 shadow-xl overflow-hidden overflow-x-auto">
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-zinc-50">
                        {Object.entries(
                            movements.reduce((acc: Record<string, MovimientoCompleto[]>, mov: any) => {
                                const area = mov.area;
                            ...
                            <AreaGroup
                                key={area}
                                area={area}
                                // 🟢 AQUÍ ESTÁ LA SOLUCIÓN: Le decimos a TypeScript exactamente qué es "movs"
                                movs={movs as MovimientoCompleto[]}
            ...
            <div className={`fixed inset-0 z-50 flex justify-end transition-all ${isDetailOpen ? 'visible' : 'invisible'}`}>
                <div className={`absolute inset-0 bg-zinc-900/40 backdrop-blur-sm transition-opacity duration-500 ${isDetailOpen ? 'opacity-100' : 'opacity-0'}`} onClick={() => { closeDetail(); setIsEditing(false); }} />
